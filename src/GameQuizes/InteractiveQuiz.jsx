    ...
    return (
      <div className="quiz-result">
        <div className="result-card">
          <div className={`trophy ${percentage > 80 ? 'gold' : percentage > 50 ? 'silver' : 'bronze'}`}>
            <FaTrophy />
          </div>
          <h2>Quiz Complete!</h2>
          <p className="score">Your Score: <span>{score}</span></p>
          <p className="percentage">{percentage}% Correct</p>
          <p className="streak">Max Streak: {maxStreak} in a row!</p>
          <div className="result-actions">
            <button onClick={restartQuiz} className="retry-btn">
              Try Again
            </button>
            <button 
              onClick={() => navigate(`/lessons/${courseId}/${moduleId}`)} 
              className="back-btn"
            >
              Back to Lessons
            </button>
